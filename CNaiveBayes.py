...
class CNaivebase():
    ...
    def Initialize(self):
        ...
        for index in range(self.totalDocument):
            current_class = self.TweetData.loc[index, 'Type']
            if pd.isna(current_class):
                continue
            terms = self.tokenize(self.TweetData.loc[index, 'Orig_Tweet'])
    ...
    def CalculateTermProbablity(self,query):
        ...
        retrive_data.update({"Tweet":className})
    ...
    def CalculateTraingAccuracy(self):
        ...
        for index in range(self.totalDocument):
            ...
            if pd.isna(current_class):
                continue
            des_query = self.TweetData.loc[index, 'Orig_Tweet']
            query_result = self.CalculateTermProbablity(des_query)
            mov_name = query_result['Tweet']
